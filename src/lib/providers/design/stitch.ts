...
export type SupportedVariants = 'MODEL_A' | 'MODEL_B' | 'MODEL_C' | 'MODEL_D';

export interface DesignProvider {
  name: string;
  generateVariants(data: OnboardingData): Promise<Record<SupportedVariants, DesignSpec>>;
  isAvailable(): boolean;
}
...
export class StitchDesignProvider implements DesignProvider {
  ...
  async generateVariants(data: OnboardingData): Promise<Record<SupportedVariants, DesignSpec>> {
    if (!this.isAvailable()) {
      throw new Error('Google Stitch API Key não configurada.');
    }

    try {
      // Chamada à API oficial do Google Stitch / Design DNA voltada a Profissionais Liberais
      const response = await fetch('https://stitch.googleapis.com/v1/projects:generateDesignDNA', {
        ...
        body: JSON.stringify({
          prompt: `Gere 4 propostas de Design DNA genéricas e customizáveis voltadas para Profissionais Liberais, adaptadas para a profissão informada: ${data.profession} (Especialidade: ${data.mainSpecialty}). Estruture layouts para: Institucional Confiável, Moderno Dark Premium, Minimalista Editorial e Consultoria/Alta Conversão.`,
          styles: ['institucional_confiavel', 'moderno_premium', 'minimalista_editorial', 'consultoria_alta_conversao'],
          brandColors: {
  ...
  private mapStitchResponseToDesignSpec(stitchResult: any, data: OnboardingData): Record<SupportedVariants, DesignSpec> {
...
export async function getDesignSpecsWithProvider(data: OnboardingData): Promise<{
  providerUsed: string;
  specs: Record<SupportedVariants, DesignSpec>;
}> {
